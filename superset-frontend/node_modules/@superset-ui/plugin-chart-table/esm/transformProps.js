...
const processColumns = memoizeOne(function processColumns(props) {
  ...
  !(rawPercentMetricsSet.has(key) && !metricsSet.has(key))).map((key, i) => {
    ...
    const dataType = coltypes[i]; // fallback to column level formats defined in datasource

    const format = columnFormats == null ? void 0 : columnFormats[key]; // for the purpose of presentation, only numeric values are treated as metrics
    ...
    if (isTime) {
      const timeFormat = format || tableTimestampFormat; // When format is "Adaptive Formatting" (smart_date)
...
const transformProps = chartProps => {
  var _queriesData$, _queriesData$2, _queriesData$2$data, _queriesData$2$data$;
  ...
  const data = processDataRecords(queriesData == null ? void 0 : (_queriesData$ = queriesData[0]) == null ? void 0 : _queriesData$.data, columns);
  const rowCount = queriesData == null ? void 0 : (_queriesData$2 = queriesData[1]) == null ? void 0 : (_queriesData$2$data = _queriesData$2.data) == null ? void 0 : (_queriesData$2$data$ = _queriesData$2$data[0]) == null ? void 0 : _queriesData$2$data$.rowcount;
  return {
    height,
    width,
