...
function normalizeStyles(nodes, links, {
  ...
  nodes.forEach(node => {
    ...
    node.symbolSize = ((node.value - nodeMinValue) / nodeSpread * NORMALIZATION_LIMITS.maxNodeSize || 0) + NORMALIZATION_LIMITS.minNodeSize; // eslint-disable-next-line no-param-reassign
  ...
  links.forEach(link => {
    // eslint-disable-next-line no-param-reassign
    link.lineStyle.width = (link.value - linkMinValue) / linkSpread * NORMALIZATION_LIMITS.maxEdgeWidth || 0 + NORMALIZATION_LIMITS.minEdgeWidth;
  });
}
